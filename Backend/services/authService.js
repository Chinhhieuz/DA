...
const generateResetPasswordToken = async (email) => {
    ...
    try {
        ...
        if (!brevoApiKey) {
            throw new Error('API Key của dịch vụ gửi Mail chưa được cấu hình. Vui lòng kiểm tra file .env');
        }
        
        // Gửi qua máy chủ tối cao Brevo API (Vượt tường lửa Render và Không giới hạn người nhận)
        ...
        // Brevo trả về code 201 nếu tạo thư thành công
        if (!response.ok) {
            throw new Error(data.message || 'Lỗi cấp quyền từ máy chủ Brevo');
        }

        return { message: 'Thành công! Email khôi phục đã qua ải chặn và nằm gọn trong hộp thư của bạn.' };
    } catch (error) {
